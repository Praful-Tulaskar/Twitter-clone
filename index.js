...
async function getTweetsAndInsertHTML() {
    ...
    document.getElementById('main-tweet-section').insertAdjacentHTML('beforeend', tweets.data.map((tweet) => {
        const date = new Date(tweet.creationDatetime);
        
        return `<div id=${tweet._id} class="tweet-body">
            ...
            <div class="tweet-header">
                ...
                <p><strong>Praful Tulaskar</strong></p>
                <p>&nbsp@Prafultulaskar</p>
...
document.addEventListener('click', async (event) => {
    if(event.target.classList.contains('tweet-btn-2')) {
        const tweetText = document.querySelector('.post-area').value;
